...
def generate_dataset_spring(args,num_sims,isTrain = True):
    loc_all = list()
    vel_all = list()
    edges = list()
    timestamps = list()
...
if args.simulation =="springs":
    ...
    spring_dir = os.path.join('simple_spring_long')
    Path(spring_dir).mkdir(parents=True, exist_ok=True)

    loc_test, vel_test, edges_test, timestamps_test = generate_dataset_spring(args, args.num_test, isTrain=False)

    np.save(os.path.join(spring_dir, 'loc_test' + suffix + '.npy'), loc_test)
    np.save(os.path.join(spring_dir,'vel_test' + suffix + '.npy'), vel_test)
    np.save(os.path.join(spring_dir,'edges_test' + suffix + '.npy'), edges_test)
    np.save(os.path.join(spring_dir,'times_test' + suffix + '.npy'), timestamps_test)

    print("Generating {} training simulations".format(args.num_train))
    loc_train, vel_train, edges_train, timestamps_train = generate_dataset_spring(args, args.num_train, isTrain=True)

    np.save(os.path.join(spring_dir, 'loc_train' + suffix + '.npy'), loc_train)
    np.save(os.path.join(spring_dir, 'vel_train' + suffix + '.npy'), vel_train)
    np.save(os.path.join(spring_dir, 'edges_train' + suffix + '.npy'), edges_train)
    np.save(os.path.join(spring_dir, 'times_train' + suffix + '.npy'), timestamps_train)
