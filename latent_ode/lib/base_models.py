...
class Baseline(nn.Module):
	...
	def get_mape(self, truth, pred_y, mask = None):
		...
		# Compute likelihood of the data under the predictions
		log_density_data = compute_mape(pred_y, truth, mask = mask)
		# shape: [1]
		return torch.mean(log_density_data)
	...
	def compute_all_losses(self, batch_dict,
		...
		mape = self.get_mape(batch_dict["data_to_predict"], pred_x,
			mask = batch_dict["mask_predicted_data"])
		...
		results["mape"] = torch.mean(mape).detach()
...
class VAE_Baseline(nn.Module):
	...
	def get_rmse(self, truth, pred_y, mask = None):
		...
		# Compute likelihood of the data under the predictions
		log_density_data = compute_rmse(pred_y, truth, mask = mask)
		# shape: [1]
		return torch.mean(log_density_data)
	...
	def compute_all_losses(self, batch_dict, n_traj_samples = 1, kl_coef = 1.):
		...
		# mape = self.get_mape(batch_dict["data_to_predict"], pred_y,
		# 	mask = batch_dict["mask_predicted_data"])

		rmse = self.get_rmse(batch_dict["data_to_predict"], pred_y,
			mask = batch_dict["mask_predicted_data"])
		...
		results["mse"] = torch.mean(mse).detach()
		results["rmse"] = torch.mean(rmse).detach()
		results["pois_likelihood"] = torch.mean(pois_log_likelihood).detach()
		results["ce_loss"] = torch.mean(ce_loss).detach()
