...
def mape(mu, data, indices = None):
	n_data_points = mu.size()[-1]

	if n_data_points > 0:
		mape = torch.abs(mu - data) / torch.maximum(1e-9*torch.ones_like(data), torch.abs(data))
		mape = mape.mean()
	else:
		mape = torch.zeros([1]).to(get_device(data)).squeeze()
	return mape
	

def compute_mape(mu, data, mask = None):
	# pdb.set_trace()
	# these cases are for plotting through plot_estim_density
	if (len(mu.size()) == 3):
		# add additional dimension for gp samples
...
def rmse(mu, data, indices = None):
	n_data_points = mu.size()[-1]

	if n_data_points > 0:
		rmse = torch.abs(mu - data)
		rmse = rmse.mean()
	else:
		rmse = torch.zeros([1]).to(get_device(data)).squeeze()
	return rmse
	

def compute_rmse(mu, data, mask = None):
	# pdb.set_trace()
	# these cases are for plotting through plot_estim_density
	if (len(mu.size()) == 3):
		# add additional dimension for gp samples
	...
	if mask is None:
		...
		# pdb.set_trace()
		# Compute the likelihood per patient so that we don't priorize patients with more measurements
		res = compute_masked_likelihood(mu, data, mask, rmse)
	return res
