...
from lib.diffeq_solver import DiffeqSolver,GraphODEFunc,ReverseGraphODEFunc



def create_LGODE_model(args, input_dim, z0_prior, obsrv_std, device):
	...
	reverse_gen_ode_func=ReverseGraphODEFunc(
		ode_func_net=ode_func_net,
		device=device).to(device)

	diffeq_solver = DiffeqSolver(gen_ode_func,reverse_gen_ode_func, args.solver, args=args,odeint_rtol=1e-2, odeint_atol=1e-2, device=device)
