...
class ParseData(object):
    ...
    def load_data(self,sample_percent,batch_size,data_type="train",cut_num=5000):
        ...
        #Encoder dataloader
        if self.mode == "extrap":
            series_list_en = self.encoder_decoder_data(loc_en,vel_en,times_en)
        else:
        ...
        # Decoder Dataloader
        if self.mode == "extrap":
            series_list_de = self.encoder_decoder_data(loc_de,vel_de,times_de)
        else:
            raise NotImplementedError
    ...
    def encoder_decoder_data(self, loc, vel, times):
