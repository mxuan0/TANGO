...
class ParseData(object):
    ...
    def load_data(self,sample_percent,batch_size,data_type="train",cut_num=5000):
        ...
        if self.mode == "interp":
            ...
            time_begin = 1
        encoder_loc_data_loader, encoder_vel_data_loader, graph_data_loader = self.transfer_data(loc_observed, vel_observed, edges,
        ...
        encoder_loc_data_loader = utils.inf_generator(encoder_loc_data_loader)
        encoder_vel_data_loader = utils.inf_generator(encoder_vel_data_loader)

        graph_data_loader = utils.inf_generator(graph_data_loader)
        decoder_data_loader = utils.inf_generator(decoder_data_loader)

        return encoder_loc_data_loader, encoder_vel_data_loader,decoder_data_loader,  graph_data_loader, num_batch
    ...
    def interp_extrap(self,loc,vel,times,mode,data_type):
        ...
        if mode =="interp":
            if data_type== "test":
                # get rid of the extra nodes in testing data.
    ...
    def transfer_data(self,loc, vel, edges, times, time_begin=0):
        ...
        for i in tqdm(range(self.num_graph)):
            loc_data_per_graph,vel_data_per_graph, edge_data, edge_size = self.transfer_one_graph(loc[i], vel[i], edges[i], times[i],
                                                                           time_begin=time_begin)
            loc_data_list.append(loc_data_per_graph)
            vel_data_list.append(vel_data_per_graph)
        ...
        loc_data_loader = DataLoader(loc_data_list, batch_size=self.batch_size)
        vel_data_loader = DataLoader(vel_data_list, batch_size=self.batch_size)
        graph_loader = DataLoader(graph_list, batch_size=self.batch_size)

        return vel_data_loader,loc_data_loader, graph_loader
    ...
    def transfer_one_graph(self,loc, vel, edge, time, time_begin=0, mask=True, forward=False):
        ...
        for i, ball in enumerate(loc):
            ...
            for j in range(loc_ball.shape[0]):
                xj_loc_feature = loc_ball[j]
                xj_vel_feature = vel_ball[j]

                x_loc.append(xj_loc_feature)
                x_vel.append(xj_vel_feature)
        ...
        x_vel = torch.FloatTensor(x_vel)
        x_loc = torch.FloatTensor(x_loc)
        ...
        graph_loc_data = Data(x=x_loc, edge_index=edge_index, edge_attr=edge_attr, y=y, pos=x_pos, edge_same=edge_is_same)
        graph_vel_data = Data(x=x_vel, edge_index=edge_index, edge_attr=edge_attr, y=y, pos=x_pos, edge_same=edge_is_same)

        edge_size = edge_index.shape[1]

        return graph_loc_data,graph_vel_data, edge_data,edge_size
